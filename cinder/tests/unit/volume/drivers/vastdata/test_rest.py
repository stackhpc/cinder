...
class TestSnapshot(VastRestTestCase):
    def test_prepare_data_cb(self):
        snapshot = vast_rest.Snapshot(self.rest)
        data = {"results": [{"id": "snapshot1"}, {"id": "snapshot2"}]}
        result = snapshot._prepare_data_cb(data)
    ...
    def test_clone_volume(self, mock_post):
        mock_post.return_value.json.return_value = {"id": "new_volume"}
        self.rest.snapshots.clone_volume(
            "snapshot_id", "subsystem_id", "target_path"
        )
        mock_post.assert_called_with(
            "snapshots/snapshot_id/clone_volume/",
            data={
                "target_subsystem_id": "subsystem_id",
                "target_volume_path": "target_path",
            },
...
class TestGlobalSnapshotStream(VastRestTestCase):
    ...
    def test_ensure_new_snapshot_stream(self, mock_request):
        ...
        mock_request.assert_called_once_with(
            "POST",
            "https://host:443/api/v4/snapshots/snapshot_id/clone/",
            verify=False,
            params=None,
            data='{"loanee_root_path": "/path", '
