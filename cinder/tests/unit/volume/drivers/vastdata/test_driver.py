...
@ddt.ddt
class VASTVolumeDriverTestCase(unittest.TestCase):
    ...
    def test_get_volume_stats(self):
        ...
        self.assertEqual(self._driver.group_stats, expected_stats)
    ...
    def test_get_volume_stats_no_refresh(self):
        ...
        self._driver.group_stats = {"cached": "data"}

        stats = self._driver.get_volume_stats(refresh=False)

        self.assertEqual(stats, {"cached": "data"})
    ...
    def test_ensure_export_not_found(self):
        ...
        mock_rest.volumes.one.return_value = None
        with patch.object(self._driver, "rest", mock_rest):
            with self.assertRaises(exception.VolumeNotFound):
                self._driver.ensure_export(self.fake_conf, volume)
    ...
    def test_initialize_connection_no_nqn(self):
        ...
        with patch.object(self._driver, "rest", mock_rest):
            with self.assertRaises(exception.VolumeDriverException):
                self._driver.initialize_connection(volume, connector)
    ...
    def test_create_volume_from_snapshot_not_found(self):
        ...
        with patch.object(self._driver, "rest", mock_rest):
            with self.assertRaises(exception.SnapshotNotFound):
                self._driver.create_volume_from_snapshot(volume, snapshot)
    ...
    def test_delete_snapshot(self):
        ...
        mock_rest.snapshots.delete.assert_called_once_with(
            name__endswith=snapshot.id
        )
